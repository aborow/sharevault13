    ...
    def get_auth_code(self, **kwarg):
        if kwarg.get('code'):
            ...
            if salesforce_id:
                ...
                payload = {'code': str(kwarg.get('code')), 'redirect_uri': redirect_uri,
                           'grant_type': 'authorization_code', 'client_id': client_id,
                           'client_secret': client_secret}
                access_token = requests.post(salesforce_id.sf_access_token_url, data=payload, headers=headers)
                if access_token:
                    ...
                    if parsed_token_response:
                        data_dict = {}
                        data_dict['sf_access_token'] = parsed_token_response.get('access_token')
                        data_dict['sf_refresh_token'] = parsed_token_response.get('refresh_token')
